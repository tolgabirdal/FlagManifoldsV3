...
def SubspaceNetHead(query, support1, support2, support_labels, n_way, n_shot, normalize=True):
    """
    ...
    assert(query.size(0) == support1.size(0) and query.size(2) == support1.size(2))
    ...
    for cc in range(tasks_per_batch*n_way):
        batch_idx = cc//n_way
        qq = query[batch_idx]
        uu, _, _ = torch.svd(torch.hstack([class_representatives1[cc].double(),class_representatives2[cc].double()]))
        uu = uu.float()
